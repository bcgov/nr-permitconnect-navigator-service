...
export const updateActivityContactController = async (
  ...
  const response = await transactionWrapper<{ updated: ActivityContact; demoted: ActivityContact | undefined }>(
    async (tx: PrismaTransactionClient) => {
      ...
      if (templateParams && navEmail) {
        let template, subject;
        const isAminChange = ActivityContactRole.ADMIN === req.body.role;
        if (isAminChange || ActivityContactRole.PRIMARY === req.body.role) {
          template = isAminChange ? teamAdminAddedTemplate(templateParams) : teamPrimaryAddedTemplate(templateParams);
          const roleString = isAminChange ? 'Amdin' : 'Primary contact';
