...
const numericTransform = Prisma.defineExtension({
  ...
  result: {
    $allModels: {
      megawatts: {
        compute: ({ megawatts }: { megawatts: Prisma.Decimal | null }): number | null => megawatts?.toNumber() ?? null
      },
      latitude: {
        compute: ({ latitude }: { latitude: Prisma.Decimal | null }): number | null => latitude?.toNumber() ?? null
      },
      longitude: {
        compute: ({ longitude }: { longitude: Prisma.Decimal | null }): number | null => longitude?.toNumber() ?? null
      },
      filesize: {
      ...
      rowNumber: {
        compute: ({ rowNumber }: { rowNumber: bigint | null }): number | null =>
          rowNumber !== null ? Number(rowNumber) : null
      }
    }
  }
});
