...
export async function up(knex: Knex): Promise<void> {
  return (
    Promise.resolve()
      ...
      // Create permit_type_source_system_kind table
      .then(() =>
        knex.schema.createTable('permit_type_source_system_kind', (table) => {
          table.specificType(
            'permit_type_source_system_kind_id',
      ...
      // Populate permit_type_source_system_kind table
      .then(async () => {
        for (const ptsk of permitTypeSystemsKinds) {
          ...
          for (const sk of ptsk.systemsKinds) {
            ...
            // Insert into permit_type_source_system_kind
            if (items.length > 0) {
              await knex('permit_type_source_system_kind').insert(items);
            }
          }
        }
      })
...
export async function down(knex: Knex): Promise<void> {
  return (
    Promise.resolve()
      ...
      // Drop permit_type_source_system_kind table
      .then(() => knex.schema.dropTableIfExists('permit_type_source_system_kind'))
  );
}
