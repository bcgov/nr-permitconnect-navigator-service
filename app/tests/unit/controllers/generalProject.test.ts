...
describe('getGeneralProjectStatisticsController', () => {
  ...
  it('calls the service with query filters then responds 200', async () => {
    ...
    } as unknown as Request<never, never, never, GetGeneralProjectStatisticsRequest>;
...
describe('searchGeneralProjectsController', () => {
  ...
  it('calls the service with search params and context then responds 200', async () => {
    ...
    searchSpy.mockResolvedValue({
      projects: [TEST_GENERAL_PROJECT_1 as GeneralProject],
      totalRecords: 1
    });

    await searchGeneralProjectsController(req, res as unknown as Response<SearchProjectResponse, LocalContext>);
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith({ projects: [TEST_GENERAL_PROJECT_1], totalRecords: 1 });
  });
