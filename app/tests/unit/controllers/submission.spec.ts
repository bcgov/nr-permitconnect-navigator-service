...
describe('getSubmissionPermitData', () => {
  ...
  // Mock service call
  const getSubmissionPermitDataSpy = jest.spyOn(submissionService, 'getSubmissionPermitData');
  ...
  it('should return 200 if all good', async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await submissionController.getSubmissionPermitData(req as any, res as any, next);
  ...
  it('calls next if the submission service fails to get permit data', async () => {
    ...
    expect(next).toHaveBeenCalledTimes(1);
    // Optionally check that `next` was called with the error
    // e.g. expect(next).toHaveBeenCalledWith(expect.any(Error));
  });
});
