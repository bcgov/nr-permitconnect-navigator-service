...
vi.mock('@/utils/constants/application', () => ({
  MIN_SEARCH_INPUT_LENGTH: 50
}));
...
describe('useProjectFormNavigator', () => {
  ...
  describe('onAssigneeInput', () => {
    ...
    it('searches by name/email if length exceeds MIN_SEARCH_INPUT_LENGTH (50)', async () => {
      ...
      const mockEvent = { target: { value: 'a'.repeat(51) } as HTMLInputElement } as unknown as IInputEvent;
      await onAssigneeInput(mockEvent);
    ...
    it('searches by email regex if length is below limit but matches regex', async () => {
      vi.mocked(utils.findIdpConfig).mockReturnValueOnce({ idp: 'idir' } as ReturnType<typeof utils.findIdpConfig>);
    ...
    it('clears options if no criteria matches', async () => {
      ...
      const mockEvent = { target: { value: 'too short' } as HTMLInputElement } as unknown as IInputEvent;
      await onAssigneeInput(mockEvent);
