...
import { ElectrificationProjectType } from '@/utils/enums/electrification';
...
<template>
  ...
  >
    ...
    <Card>
      ...
      <template #content>
        <RadioList
          ...
          @on-change="
            (e: string) => {
              if (e === ElectrificationProjectType.NCTL || e === ElectrificationProjectType.OTHER)
                setFieldValue('bcHydroNumber', null);
            }
          "
        />
      </template>
    ...
    <Card v-if="values.projectType === 'IPP Wind' || values.projectType === 'IPP Solar'">
      <template #title>
        <span
          class="section-header"
          role="heading"
