...
export function useProjectFormNavigator(
  ...
  async function createATSClientEnquiry() {
    try {
      ...
      const clientErrorMsg = t(`${tPrefix}.atsClientPushError`);
      toast.error(`${clientErrorMsg} ${error}`);
    }
  }
  ...
  async function createATSEnquiry(atsClientId?: number) {
    try {
      const ATSEnquiryData: ATSEnquiryResource = {
        ...
        clientId: atsClientId ?? formRef.value?.values.atsClientId,
      ...
      const errorMessage = t(`${tPrefix}.atsEnquiryPushError`);
  ...
  const onAssigneeInput = async (e: IInputEvent) => {
    ...
    if (idpCfg) {
      if (new RegExp(Regex.EMAIL).exec(input)) {
